    ...
    try {
        ...
        for (const templateName of templateNames) {
            ...
            try {
                ...
                const htmlContent = await fs_1.promises.readFile(filePath, 'utf8');
 ...
 */
const getEmailHtml = async (templateName, templateVariables = {}) => {
    try {
 ...
 */
const createFallbackHtml = (templateName, variables) => {
      ...
      <body>
        <div class="container">
          ...
          <div style="text-align: center;">
            <a href="${variables.addIdeaUrl || env_1.env.WEBAPP_URL + '/ideas/new'}" class="btn">➕ Share Your First Idea</a>
          </div>
          <div class="footer">
            <p>&copy; 2024 IdeaNick. All rights reserved.</p>
            <p><a href="${variables.homeUrl || env_1.env.WEBAPP_URL}">${variables.homeUrl || env_1.env.WEBAPP_URL}</a></p>
          </div>
        </div>
      </body>
      </html>
    `;
      ...
      <body>
        <div class="container">
          ...
          <div style="text-align: center;">
            <a href="${variables.homeUrl || env_1.env.WEBAPP_URL}" class="btn">Visit IdeaNick</a>
          </div>
          <div class="footer">
            <p>&copy; 2024 IdeaNick. All rights reserved.</p>
            <p><a href="${variables.homeUrl || env_1.env.WEBAPP_URL}">${variables.homeUrl || env_1.env.WEBAPP_URL}</a></p>
 ...
 */
const sendEmail = async ({ to, subject, templateName, templateVariables = {}, }) => {
    try {
        const fullTemplateVariables = {
            ...templateVariables,
            homeUrl: env_1.env.WEBAPP_URL,
        };
        ...
        const { loggableResponse } = await (0, brevo_1.sendEmailThroughBrevo)({ to, html, subject });
        logger_1.winstonLogger.info('email', '✅ sendEmail успешно отправлен', {
            to,
            templateName,
            templateVariables,
        ...
        logger_1.winstonLogger.error('email', error, {
            to,
            templateName,
            templateVariables,
        });
